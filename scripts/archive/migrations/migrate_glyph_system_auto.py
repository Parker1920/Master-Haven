"""
Database Migration Script: Add Glyph System Support (Non-Interactive)

IMPORTANT: Stop the web server before running this script!
...
5. Clear all test data automatically
"""
...
def migrate_database():
    ...
    try:
        ...
        if 'region' in current_columns and 'galaxy' not in current_columns:
            ...
            cursor.execute("""
                INSERT INTO systems_new (id, name, x, y, z, galaxy, fauna, flora, sentinel, materials, base_location, photo, attributes, created_at, modified_at)
                SELECT id, name, x, y, z, region, fauna, flora, sentinel, materials, base_location, photo, attributes, created_at, modified_at
                FROM systems
            """)
        ...
        columns_to_add = {
            'glyph_code': 'TEXT',  # Can't add UNIQUE with ALTER, will add index later
        ...
        cursor.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_systems_glyph ON systems(glyph_code) WHERE glyph_code IS NOT NULL")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_systems_galaxy ON systems(galaxy)")
        print("  [OK] Spatial indexes created")

        # Step 5: Clear test data (automatic)
        ...
        print(f"  Found {old_systems_count} systems, {old_planets_count} planets, {old_discoveries_count} discoveries")
...
if __name__ == '__main__':
    ...
    print("WARNING: This will clear all existing test data!")
    print("Make sure the web server is stopped before continuing.\n")
