...
- The difference between TCP (reliable, ordered, verified delivery — used for web traffic, SSH) and UDP (fast, fire-and-forget — used for VPNs, video streaming, gaming)
- Docker Compose basics: `docker compose up -d` (start detached), `docker compose down` (stop), `docker ps` (list running), `docker logs <container>` (view logs), `docker stats` (resource usage)
- How DNS propagation works — when you change nameservers, the internet needs time to learn about the change; Cloudflare shows "Pending" until it detects the nameserver update
- What NAT hairpinning is (also called NAT loopback) — when you try to access your own public IP from inside your network, consumer routers often can't handle routing that traffic back inside. Symptom: external access works, local access times out. Fix: use hosts file or local DNS (like Pi-hole) to point the domain directly to the internal IP
- How Docker networking differs from host networking — `localhost` inside a Docker container refers to the container itself, not the host machine. To reach services on the host, use the host's actual IP address (e.g., 10.0.0.33)
- How the Windows hosts file works (`C:\Windows\System32\drivers\etc\hosts`) — a local DNS override that maps domain names to IP addresses before consulting DNS servers
...
NPM sees the hostname is `havenmap.online` and has a rule: forward to `10.0.0.33:8005` (the Pi's IP, not localhost — because NPM runs in Docker where localhost would mean the container itself). It creates a new HTTP request (unencrypted, but this is fine — it never leaves the Pi) and sends it to Haven's Python web server on port 8005. Haven processes the request, generates the HTML response, and sends it back to NPM.
...
**Current configuration (Phase 1 — 2GB Pi hosts everything):**
| External Port | Destination IP | Destination Port | Protocol | Purpose | Status |
|---------------|---------------|------------------|----------|---------|--------|
| 80 | 10.0.0.33 (2GB Pi) | 80 | TCP | HTTP → NPM | ✅ Configured |
| 443 | 10.0.0.33 (2GB Pi) | 443 | TCP | HTTPS → NPM | ✅ Configured |
| 51820 | 10.0.0.33 (2GB Pi) | 51820 | UDP | WireGuard VPN (future) | Not yet needed |

**Future configuration (after 8GB Pi purchased — update port forwards to point to 8GB Pi for 80/443):**
| External Port | Destination IP | Destination Port | Protocol | Purpose |
|---------------|---------------|------------------|----------|---------|
| 80 | 8GB Pi static IP (e.g., 10.0.0.34) | 80 | TCP | HTTP → NPM |
| 443 | 8GB Pi static IP | 443 | TCP | HTTPS → NPM |
| 51820 | 10.0.0.33 (2GB Pi) | 51820 | UDP | WireGuard VPN |
...
4. Create A record in Cloudflare pointing to home IP ✅ (DDNS container auto-created it)
5. Install Docker on 2GB Pi ✅
6. Deploy NPM + DDNS containers from the compose file ✅
...
**Phase 1 is COMPLETE.** Ready to proceed to Phase 2 when Parker is ready.

#### Phase 1 Progress Log (February 2, 2026)

| Step | Status | Notes |
|------|--------|-------|
| Cloudflare account | ✅ Done | Account created, domain added |
| Cloudflare API token | ✅ Done | Scoped to havenmap.online zone, stored securely |
| Namecheap nameservers | ✅ Done | Changed to noel.ns.cloudflare.com, dina.ns.cloudflare.com |
| DNS propagation | ✅ Done | Cloudflare shows "Active" |
| Docker on Pi | ✅ Done | Docker + Docker Compose plugin installed |
| docker-compose.yml | ✅ Done | Located at `~/docker/docker-compose.yml` with NPM + DDNS |
| Port forwarding | ✅ Done | 80→Pi:80, 443→Pi:443 via xFi app |
| NPM running | ✅ Done | Accessible at `http://10.0.0.33:81`, credentials changed |
| DDNS running | ✅ Done | Auto-created A record pointing to public IP |
| A record | ✅ Done | DDNS container created it automatically (no manual creation needed) |
| NPM proxy host | ✅ Done | havenmap.online → 10.0.0.33:8005, SSL enabled, Force HTTPS on |
| External test | ✅ Done | Phone on cellular confirmed working with green padlock |
| NAT hairpinning fix | ✅ Done | Added hosts file entry on Windows PC (see below) |

**Phase 1 Complete!** Haven is now accessible at `https://havenmap.online` with valid SSL.

#### NAT Hairpinning Issue & Fix

**Problem encountered:** After Phase 1 was fully configured, the site worked perfectly from external networks (phone on cellular) but timed out when accessed from Parker's home PC.

**Root cause:** NAT hairpinning (also called NAT loopback). When a device inside the network tries to access the public IP of its own router, the Xfinity gateway doesn't know how to loop that traffic back inside. The request goes out to the router's public interface and gets stuck. This is a limitation of many consumer routers.

**Solution:** Bypass public DNS for local access by adding an entry to the Windows hosts file:

```
# Location: C:\Windows\System32\drivers\etc\hosts
# (Must edit with Administrator privileges)

10.0.0.33    havenmap.online
```

This tells Windows: "Don't bother asking DNS servers for havenmap.online — I already know it's at 10.0.0.33." The request goes directly to the Pi on the local network, avoiding the NAT hairpinning problem entirely.

**Trade-off:** This entry only exists on Parker's Windows PC. Any new devices on the home network will need the same hosts file entry, OR Parker can set up Pi-hole (Phase 3) to handle local DNS resolution for the whole network.

**Lesson learned:** External access working + local access broken = classic NAT hairpinning symptom.
...
#### 2GB Pi: Current Configuration (Phase 1)

Currently deployed at `~/docker/docker-compose.yml` on the 2GB Pi:
...
services:
  ...
  cloudflare-ddns:
    ...
    network_mode: host
    environment:
      - CF_API_TOKEN=<parker's token - not committed to repo>
      - DOMAINS=havenmap.online
      - PROXIED=false
      - IP6_PROVIDER=none
    restart: unless-stopped
```
...
Once the 8GB Pi is purchased, NPM and Haven will move there. The 2GB Pi will become dedicated network infrastructure:
